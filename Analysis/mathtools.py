# -*- coding: utf-8 -*-
"""
Solve root of 4th order polynoms
Inspired from https://github.com/NKrvavica/fqs
...
def cubic_roots(p):
    ...
    if p.shape[1] != 4:
        raise ValueError("Expected 3rd order polynomial with 4 coefficients, got {:d}.".format(p.shape[1]))
...
def quartic_roots(p):
    ...
    if p.shape[1] != 5:
        raise ValueError("Expected 4th order polynomial with 5 coefficients, got {:d}.".format(p.shape[1]))

    roots = multi_quartic(*p.T)
    return np.array(roots).T


def locmin(val, depth=None, fillmin=False, retdepth=False, retpos=False, axis=-1):
    """
    Find local minimum (point for which near points have higher values)
    Get minimal value and (optionally) depth

    val: np array float
        Value sin which to find local minimum
    depth: np array float
        corresponding depth to be retruned (location of the minimum)
    fillmin: bool
        fill with absolute minimùum if no local minimum found, default False
        when no minimim found, return np.nan
    retdepth: bool
        return depth value in addition to local min value
    retpos: bool
        return indices of minimum found (as usable by take_along_axis)
    axis: int
        axis on which to find the local minimum, default -1

    returns: np array [, np array [, np array]]
        min vales and (optionally) corresponding depth values and locations
        np.nan when no minimum found
    """
    if depth is None and retdepth:
        raise ValueError("retdepth turned to True while no giving depth")

    shape = val.shape

    def get_slices(axis, *args):
        slices = [slice(None, None, None)] * len(shape)
        slices[axis] = slice(*args)
        return tuple(slices)

    mask = np.full_like(val, True, dtype=bool)
    # Unmask local minima
    mask[get_slices(axis, 1, -1)] = ~((val[get_slices(axis, 1, -1)] < val[get_slices(axis, None, -2)]) & (val[get_slices(axis, 1, -1)] < val[get_slices(axis, 2, None)]))
    val_m = np.ma.array(val, mask=mask)
    min_v = val_m.min(axis=axis)
    if len(shape) == 1:  # return a unique value
        min_v = np.array([min_v])
    else:
        min_v = min_v.filled(np.nan)
    if retdepth or retpos:
        pos = np.expand_dims(val_m.argmin(axis=axis), axis=axis)
    if retdepth:
        depth_s = np.take_along_axis(depth, pos, axis=axis)
        if len(shape) > 1:
            depth_s = np.take(depth_s, 0, axis=axis)
        depth_s[np.isnan(min_v)] = np.nan

    if fillmin:
        select = np.isnan(min_v)
        min_v[select] = np.nanmin(val, axis=axis)[select]
        if retdepth or retpos:
            select2 = select & np.isfinite(min_v)
            loc_min = np.expand_dims(np.nanargmin(val[select2, :], axis=axis), axis=axis)
            if retdepth:
                depth_s2 = np.take_along_axis(depth[select2, :], loc_min, axis=axis)
                if len(shape) > 1:
                    depth_s[select2] = np.take(depth_s2, 0, axis=axis)
                elif np.isnan(depth_s).all():
                    depth_s = depth_s2
            if retpos:
                pos[select2] = loc_min
...
def get_val_pos(val, pos, axis=-1, origval=None):
    """
    Get values of val at a given position
    (for instance as returned by locmin)

    val: np array
        Value to extract
    pos: np array (same nb dimensions)
        Positions to extract on axis
    axis: int
        Axis, default -1
    origval: np array
        if provided, remove extracted values
        when origval is nan
    """
    r = np.take_along_axis(val, pos, axis=-1)
    if len(r.shape) > 1:
        r = np.take(r, 0, axis=axis)
    if origval is not None:
        r[np.isnan(origval)] = np.nan
    return r
